<template>
  <Teleport to="body">
    <Transition name="failed">
      <div v-if="wasFailed" ref="failedEffect" @click="$emit('hideEffect')" @keydown.enter="$emit('hideEffect')" @keydown.esc="$emit('hideEffect')" tabindex="0" class="fixed top-0 w-full h-screen bg-black bg-opacity-30 flex flex-col place-content-center cursor-pointer">
