<template>
  <div class="lg:sticky top-0 h-max sm:flex sm:gap-8 justify-center lg:block text-center bg-zinc-900 bg-opacity-60 p-2">
    <!-- Known game -->
    <div class="sm:inline-block lg:block mb-2 sm:mb-0">
      <p class="text-sm">Game</p>
      <Transition name="known" mode="out-in" class="sm:h-11 sm:leading-[2.2] whitespace-nowrap text-xl lg:text-2xl">
        <p :key="known.game">{{ known.game }}</p>
      </Transition>
    </div>
    <!-- Known health & souls container -->
    <div class="grid grid-cols-2 sm:flex sm:gap-8 lg:block mb-2 sm:mb-0">
      <!-- Known health -->
      <div class="sm:inline-block lg:block">
        <p class="text-sm">Health</p>
        <p v-if="known.health" class="sm:h-11 sm:leading-[2.2] text-lg xs:text-xl lg:text-2xl">{{ known.health.toLocaleString() }}</p>
      ...
      <div class="sm:inline-block lg:block">
        ...
        <p v-if="known.souls" class="sm:h-11 sm:leading-[2.2] text-lg xs:text-xl lg:text-2xl">{{ known.souls.toLocaleString() }}</p>
        <p v-else class="sm:h-11 sm:leading-[2.2] text-lg xs:text-xl lg:text-2xl">
          <Transition name="known" mode="out-in">
    ...
    <div class="grid grid-cols-2 sm:flex sm:gap-8 lg:block">
      <!-- Known weaknesses -->
